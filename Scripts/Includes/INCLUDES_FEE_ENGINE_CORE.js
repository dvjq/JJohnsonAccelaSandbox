    ...
    try {
        ...
        this.calcFee = function (fi) {
            ...
            if (getFeeResult.getSuccess()) {
                ...
                for (var feeNum in feeList) {
					...
					var financeFeeItems = aa.finance.getFeeItemByPK(_itemCap, feeList[feeNum].getFeeSeqNbr()).getOutput().getF4FeeItem();
                    var getFeeNotes = financeFeeItems.getFeeNotes();
                    if (feeList[feeNum].getFeeitemStatus().equals("INVOICED") && getFeeNotes !=null) {
            ...
            if ((refFee && updateByAmount && _updateByAmtTypes.indexOf(String(refFee.getCalProc())) >= 0) || fi.customCalcFunction || fi.offsetAmt || fi.offsetPct) {
                ...
                //Set fi.invoice back from holdInvoice
                fi.invoice = holdInvoice;
        ...
        this.getDefaultVersionByScheduleAndFeeCode = function (fi, vDate) {
            ...
            var sql = "SELECT FEE_SCHEDULE_VERSION " +
                ...
                " AND eff_date <= CONVERT(date, '" + effDateString + "', 101) " +
                " AND rfee.rec_status = 'A' " +
                " AND ( exp_date IS NULL OR exp_date >= CONVERT(date, '" + effDateString + "', 101)) " +
			   ...
			   var sStmt = aa.db.prepareStatement(conn, sql);
                ...
                if (sql.toUpperCase().indexOf("SELECT") == 0) {
                    ...
                    while (rSet.next()) {
                        ...
                        for (i = 1; i <= columns; i++) {
                            obj[md.getColumnName(i).toUpperCase()] = String(rSet.getString(md.getColumnName(i)));
                        }
                ...
                if (array[0]) {
                    logDebug("::feeEngine.getDefaultVersionByScheduleAndFeeCode() returning version : " + array[0].FEE_SCHEDULE_VERSION);
                    return array[0].FEE_SCHEDULE_VERSION;
                } else {
                    logDebug("::feeEngine.getDefaultVersionByScheduleAndFeeCode() did not find a matching fee version.");
