...
if (matches(waitTime, null, undefined, "")) {
	waitTime = 10000;
} else {
...
function sendNotificationLocal(emailFrom, emailTo, emailCC, templateName, params, reportFile) {
	...
	var result = null;
	if (isEmptyOrNull(emailFrom)) emailFrom = null;
	if (isEmptyOrNull(emailCC)) emailCC = null;
...
function runReportAttach(itemCapId, aaReportName) {
	...
	if (!reportResult.getSuccess()) {
		asyncDebug("**WARNING** couldn't load report " + reportName + " " + reportResult.getErrorMessage());
		return false;
	}
...
function getLicensedProfessionalObjectsByRecord(pCapId, licenseTypeArray) {
	var itemCap = capId;
	if (pCapId != null) itemCap = pCapId; // use cap ID specified in args
		...
		if (licenseProfList) {
			for (thisLP in licenseProfList) {
				if (licenseProfList[thisLP].getLicenseNbr() != null) {
					if (!licenseTypeArray || exists(licenseProfList[thisLP].getLicenseType(), licenseTypeArray)) var vLPObj = new licenseProfObject(licenseProfList[thisLP].getLicenseNbr());
		...
		try {
			...
			if (rNotifyContactType[0].toUpperCase() != "ALL" && rNotifyContactType[0].toUpperCase() != "PRIMARY") {
					...
					for (y in capContactArray) {
						...
						for (z in rNotifyContactType) {
							if (thisCon.getPeople().getContactType().toUpperCase() == rNotifyContactType[z].toUpperCase()) {
								if (thisCon.getEmail()) contactObjArray.push(new contactObj(thisCon));
							}
						}
					}
				}
				...
				if (capContactArray) {
					for (y in capContactArray) {
						thisCon = capContactArray[y];
						if (thisCon.getPeople().getFlag() == "Y" && thisCon.getEmail()) contactObjArray.push(new contactObj(thisCon));
					}
				}
			}
...
function convertStringToHashMap(theString) {
	...
	for (var i = 0, cur, pair; (cur = theString[i]); i++) {
		...
		if (pair[1]) var val = pair[1].toString();
		else val = "";
		retObj.put(key, val);
	}
	return retObj;
...
function matches(eVal, argList) {
	for (var i = 1; i < arguments.length; i++) if (arguments[i] == eVal) return true;
}
