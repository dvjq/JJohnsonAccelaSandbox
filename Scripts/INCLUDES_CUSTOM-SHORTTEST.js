...
function autoScheduleInspection(vCapId, inspSeqNbr, date) {
    ...
    if (assinfo.flag == "S") {
        ...
        if (!editInspResult.getSuccess()) {
            ...
            logDebug("Successfully re-assigned inspection " + inspScriptModel.getInspectionType() + " to user " + inspector.getUserID() + " on " + schedDate);
        }
        return true;
    }

    if (assinfo.flag == "C") {
        logDebug("WARNING: Cut off time will not allow to schedule.");
    ...
    if (assinfo.flag == "F") {
        ...
        switch (assinfo.resultType) {
            ...
            case 14:
                errMsg = "Scheduled time is not available";
                break;
            case 15:
...
// *** BEGIN TIME ACCOUNTING ***

/* 
###### Begin Time Accounting Functions #####
*/ 
// This function takes all unlocked billable time accounting entries and attempts to assess/invoice associated fees
...
function isDPRRecord(recordType) {
	var isRecordFound = false;
	var bizDomScriptResult = aa.bizDomain.getBizDomainByValue("DPR_RECORD_TYPES_ENABLED",recordType);
	if (bizDomScriptResult.getSuccess()) {
		var bizDomainValue = bizDomScriptResult.getOutput();
		if (bizDomainValue.getAuditStatus() == "A") {
			isRecordFound = true;
		}
	}
...
function isDprDisciplineTask(recordType, taskName) {

	var isPlanReviewTask = false;
	...
	return isPlanReviewTask;
}

function addPlanRoomCondition() { //optional capId
	...
	var plansTabUrl = lookup("DPR_CONFIGS", "PLANS_TAB_URL");
	if (plansTabUrl == undefined) {
		plansTabUrl = "/av-dpr/sheets.html";
	}
	
	if (conditionConfig != undefined) {
		var conditionConfigArray = conditionConfig.split("|");

		var hasPlanRoomCondition = appHasCondition(conditionConfigArray[0],null,conditionConfigArray[3],null);
		if (hasPlanRoomCondition) {
			// set condition to applied
			var editConditionResult = editCapConditionStatus(conditionConfigArray[0],conditionConfigArray[3],conditionConfigArray[1],null);	
		} else {
			// Add app condition
			addAppCondition(conditionConfigArray[0], conditionConfigArray[1], conditionConfigArray[3], "<a href=\"" + plansTabUrl+ "?id=" + capId.getId() + "&customId=" + capIDString + "&back=false\"  rel=\"noopener noreferrer\" target=\"_blank\">Plan Room Project</a>", conditionConfigArray[2]);

			// update the condition to viewable - set this condition to number 1
		...
		// get all app conditions and re-order
		var conditionReOrderArray = [];
		var allCapConditionsResult = aa.capCondition.getCapConditions(itemCap);
		if (allCapConditionsResult.getSuccess()) {
			var allCapConditions = allCapConditionsResult.getOutput();
			var remainingConditions = [];
			// Get the plan room condition first
			for (var c in allCapConditions) {
				thisCondition = allCapConditions[c];
				if (thisCondition.getConditionType() == conditionConfigArray[0]) {
					conditionReOrderArray.push(thisCondition);
				} else {
					remainingConditions.push(thisCondition);
				}
			}
			// push the remaining conditions into the re-order array
			if (remainingConditions.length > 0) {
				conditionReOrderArray = conditionReOrderArray.concat(remainingConditions);
			}

			// re-order the conditions
			if (conditionReOrderArray.length > 0) {
				var capConditionDAO = aa.proxyInvoker.newInstance("com.accela.aa.aamain.cap.CapConditionDAOOracle").getOutput();
				for (var croa in conditionReOrderArray) {
					cond = conditionReOrderArray[croa];
					var displayOrder = parseInt(croa) + 1;
					capConditionDAO.reOrderCapCondition(itemCap, aa.util.parseLong(cond.getConditionNumber()), aa.util.parseLong(displayOrder), aa.util.parseDate(sysDateMMDDYYYY), currentUserID);
				}
			}
		}		
	}
...
// overriding standard function while urlrouting is not working
...
function reconcileBuildingPlans() {
    // function to update document types for building plans
    // upon final approval

    // get document type configuration from standard choice
    var bizDomScriptResult = aa.bizDomain.getBizDomain("DPR_PRINTSET_RECONCILE");
    if (bizDomScriptResult.getSuccess()) {
        var bizDomScript = bizDomScriptResult.getOutput();
        var docTypesConversionArray = new Array();
        var isSCConfigured = false;
        for (var i = 0; i < bizDomScript.size(); i++) {
            var sc = bizDomScript.get(i);
            if (sc.getAuditStatus() != "A") {
                continue;
            ...
            var scValue = sc.getBizdomainValue();
            var scValueDesc = sc.getDescription();
            docTypesConversionArray[scValue] = scValueDesc
        }
        
        for (var prop in docTypesConversionArray) {
            if(docTypesConversionArray.hasOwnProperty(prop)) {
                isSCConfigured = true;
                break;
            }
        }
        ...
        if (isSCConfigured > 0) {
            var recordDocumentList = getDocumentList();
            if (recordDocumentList.length > 0) {
                for (var i in recordDocumentList) {
                    var thisDoc = recordDocumentList[i];
                    if (docTypesConversionArray.hasOwnProperty(thisDoc.getDocCategory())) {
                        thisDoc.setDocCategory(docTypesConversionArray[thisDoc.getDocCategory()]);
                        aa.document.updateDocument(thisDoc);                        
                    }
            ...
            logDebug("Unable to retrieve printset reconciliation configuration from standard choice");
        }
    } else {
        logDebug("Unable to retrieve printset reconciliation configuration from standard choice");
    }
...
/// CONFIGURABLE SCRIPTS EMERGENCY FIX - -- REMOVE LATER
/*------------------------------------------------------------------------------------------------------/
